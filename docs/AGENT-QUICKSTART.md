...
4. Ask for the powered cabinet. Call `blackout_v3_tune_decoder` with the recorded shift, decode the label backward, and message the word.
...
7. After power restoration, `blackout_v3_control_environment` may set LIGHTING to EMERGENCY, WORK or ULTRAVIOLET and VENTILATION to INTAKE, EXHAUST or HOLD. These controls reveal optional story details; an unsafe valid ventilation choice costs time and can be corrected.
...
Errors include a category and retryable flag. `STALE_ROOM` means observe again, `WRONG_STAGE` means wait for the human, `INVALID_ARGUMENTS` means correct the schema, and `ROOM_CLOSED` means stop.
